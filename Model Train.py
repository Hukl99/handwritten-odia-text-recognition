...
def fit_model():
    checkpointer = ModelCheckpoint(filepath='model.weights.best.keras', verbose=1,
                                   save_best_only=True) #for keras  model creation
    
    
    #for hg.weights creation
    """checkpointer = ModelCheckpoint(
    filepath='model.weights.best.weights.h5',  # <- correct extension
    verbose=1,
    save_best_only=True,
    ...
    model_hist = model.fit(X_train, Y_train, batch_size=16, epochs=3, callbacks=callbacks_list,
                           steps_per_epoch=int(len(X_train) / 16), validation_data=(X_test, Y_test))
...
model.save('model.weights.best.keras')  #To save keras model
#model.load_weights('model.weights.best.weights.h5')  #Toload h5 model weights
